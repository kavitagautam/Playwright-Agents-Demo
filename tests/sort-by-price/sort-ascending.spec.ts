...
test.describe('Sort by Price', () => {
  test('Sort items by price ascending', async ({ page }) => {
    await page.goto('https://practiceautomatedtesting.com/shopping');

    // Click Sort by Price button to ensure ascending order
    await page.getByRole('button', { name: /Sort by Price/ }).click();
    
    // Wait for sort to be applied and products to be visible
    await page.waitForSelector('div.cursor-pointer h3');

    // Get prices and verify ascending order
    const prices = await productPriceLocator(page);
    console.log('Prices in ascending order:', prices);
    
    expect(prices.length).toBeGreaterThan(0);
    
    // Validate that each consecutive price is numerically greater than or equal to the previous
